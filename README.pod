...
B<vfnmake> [ B<-f> I<file> ] [ B<-d> I<dir> ] [ options ]
...
Enable/disable compatibility mode with the older versions of GNU Make. It effectively disables debugging options.
...
The config files are read from F</etc/vfnmake.conf>, F<~/.vfnmake.conf> and F<vfnmake.conf> in that order. The F<vfnmake.conf> in the working directory is automatically modified on each run. Additionaly there are two other files - F<vfnmake.begin> and F<vfnmake.end>. Their role is described below.
...
Just like F<vfnmake.begin> but is put at the end of the Makefile. Can be used to add some custom targets without affecting the default one.
...
You can add B<-r> to turn vfnmake into the automagic make - it will run the generated Makefile afterwards:
...
=item * the compiler is not selected by vfnmake; use "export CC=your_C_compiler" and "export CXX=your_C++_compiler" before running make
...
If you do need any of the removed features, email me, I'll what I can do.
