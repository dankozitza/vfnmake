...
Enable Assembly support (only nasm). Default: --no-asm
...
The main config file will be generated with all available options on the first run. It has the following syntax:

    OPTION: some_spaces VALUE(s)
    ...
