...
vfnmake works on fire-and-forget basis with as few configuration as possible. It does not try to compete with CMake or Autotools. I wrote it because writting Makefiles, while not difficult, can be really tedious even for the simple projects. To enable linking with Assembly, use B<--asm>/B<--asm64> options.
...
Run make automatically. Specify it twice to run the compiled program too and thrice to recompile it completely (make fresh) before running.
